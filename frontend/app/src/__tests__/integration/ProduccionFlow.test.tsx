...
function setupMocks() {
  const fetchMock = vi.fn(async (input: RequestInfo | URL, init?: RequestInit) => {
    ...
    if (url.includes("/api/n12/op") && !url.includes("/detalle")) {
      return {
        ok: true,
        json: async () => [],
      } as any;
...
describe("DashboardTaller Flow", () => {
  ...
  it("debe abrir el modal de detalle con la información técnica completa", async () => {
    ...
    // Tabla: {p.h} x {p.w} cm (mock: w=240, h=60 -> "60 x 240 cm")
    await waitFor(() => {
      expect(withinDialog.getAllByText(/60\s*x\s*240/).length).toBeGreaterThan(0);
    });
