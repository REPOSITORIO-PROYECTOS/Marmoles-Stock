...
import {
  ...
  Layers, Maximize2, Tag, CheckCircle2, Info, FileText, MapPin, Phone, Mail, Image as ImageIcon, Box, Trash2, Filter, Play
} from 'lucide-react';
...
import { LienzoDeCorte } from '../../components/produccion/LienzoDeCorte';
...
const COLUMNAS: { id: EstadoProduccion; titulo: string; color: string }[] = [
  ...
  { id: 'corte', titulo: 'En Corte', color: 'bg-cyan-50' },
  ...
  const handleImprimirEtiquetas = () => {
    ...
    piezas.forEach(pieza => {
      html += `
        <div class="etiqueta">
          <div class="header">
            <div class="title">MUNDO DI MARMI - ETIQUETA DE PIEZA</div>
      ...
      <div className="flex items-start justify-between mb-3">
        <div className="flex items-center gap-2">
          ...
          <div className="flex flex-col">
            ...
            <span className="text-primary font-mono text-xs font-bold bg-cyan-50 px-1 rounded">{orden.presupuesto}</span>
          </div>
        </div>
      ...
      <div className="space-y-1 mb-3">
        {orden.acumuladoId && (
          <div className="flex items-center gap-1 mb-2">
            <Badge variant="secondary" className="bg-cyan-50 text-cyan-700 border-cyan-100 text-[9px] h-4">
...
function DashboardTallerContent() {
  ...
  // Estado para el lienzo de corte
  ...
  return (
    <div className="h-full flex flex-col bg-slate-50/50">
      ...
      <Dialog open={!!ordenSeleccionada} onOpenChange={() => setOrdenSeleccionada(null)}>
        <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
              ...
              <div className="space-y-6">
                    ...
                    <div className="space-y-2 text-sm">
                      ...
                      {ordenDetalle.cliente.email && (
                        <div className="grid grid-cols-[20px_1fr] gap-2 items-center">
                          ...
                          <a href={`mailto:${ordenDetalle.cliente.email}`} className="text-cyan-600 hover:underline">{ordenDetalle.cliente.email}</a>
                        </div>
                      )}
                    </div>
                  ) : (
                      ...
                      <div className="grid grid-cols-2 gap-4">
                        ...
                        {ordenDetalle.lote.foto_url && (
                          <div className="col-span-2 mt-2">
                            ...
                            <img
                              src={`${API_BASE}${ordenDetalle.lote.foto_url}`}
                              alt="Lote"
                ...
                {ordenDetalle?.plano ? (
                  <div className="bg-cyan-50 p-4 rounded-lg border border-cyan-100">
                    ...
                    <p className="text-xs text-cyan-600 mb-3">Versión oficial para producción</p>
                    <div className="flex gap-2">
                      <Button size="sm" variant="outline" className="bg-white" onClick={() => window.open(ordenDetalle.plano.imagen_url ? `${API_BASE}${ordenDetalle.plano.imagen_url}` : '#', '_blank')}>
                        <Eye className="h-3 w-3 mr-1" /> Ver Plano
                      </Button>
                      {ordenDetalle.dxf_url && (
                        <Button size="sm" variant="outline" className="bg-white" onClick={() => window.open(`${API_BASE}${ordenDetalle.dxf_url}`, '_blank')}>
          ...
          <DialogFooter>
            ...
            {/* Botón Procesar Corte - visible solo en estado "listo_para_corte" */}
            {(ordenSeleccionada?.estado === 'listo_para_corte' || ordenSeleccionada?.estado === 'en_acumulador') && (
              <Button
                className="gap-2 bg-blue-600 hover:bg-blue-700"
                onClick={() => setLienzoOpen(true)}
              >
                <Play className="h-4 w-4" /> Procesar Corte
              </Button>
            )}
      ...
      {/* LIENZO DE CORTE MODAL */}
      {ordenSeleccionada && (
        <LienzoDeCorte
          acumuladoId={ordenSeleccionada.acumuladoId || ordenSeleccionada.id}
          acumuladoDimensiones={{
            ancho: 2000, // Valores por defecto - idealmente vendrían del backend
            alto: 1000,
          }}
          open={lienzoOpen}
          onOpenChange={setLienzoOpen}
          onProcesado={(resultado) => {
            toast.success(`Corte completado: ${resultado.cantidad_retazos} retazos creados`);
            setLienzoOpen(false);
