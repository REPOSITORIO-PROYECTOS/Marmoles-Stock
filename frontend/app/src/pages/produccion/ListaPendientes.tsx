...
import { N12OptimizacionPanel } from "../../components/produccion/N12OptimizacionPanel";
...
const normalizeToMm = (value?: number): number => {
  const parsed = Number(value || 0);
  if (!Number.isFinite(parsed) || parsed <= 0) return 0;
  return parsed < 1000 ? Math.round(parsed * 10) : Math.round(parsed);
};

const formatDualUnit = (mm: number): string => {
  const cm = mm / 10;
  ...
  return (
    <div className="p-8">
      <Tabs defaultValue="legacy" className="mb-8">
        <TabsList>
          <TabsTrigger value="legacy">Acumuladores (trabajos)</TabsTrigger>
          <TabsTrigger value="n12">Nivel 12 — Optimización</TabsTrigger>
        </TabsList>
        <TabsContent value="n12" className="mt-4">
          <N12OptimizacionPanel />
        </TabsContent>
        ...
        <div className="flex-1">
          ...
          <Tabs defaultValue="agrupar" className="space-y-6 flex-1">
            ...
            <TabsContent value="agrupar" className="space-y-6">
              ...
              {grupos.map((g) => {
                const allValid = g.trabajos.every(t => t.visita_tecnica && t.aprobado_jefe && t.sena_abonada && t.medidas_corregidas);
                return (
                      ...
                      <div className="flex items-center justify-between">
                        ...
                        <div className="flex items-center gap-3">
                          ...
                          <Button
                            onClick={() => {
                              if (!allValid) {
                                toast.error("Algunos trabajos no cumplen la Regla de Oro (Visita + Medidas + Seña + Aprobación)");
                                return;
                              }
                      ...
                      {g.trabajos.map((t) => (
                        <div key={t.id} className="border rounded-lg p-4 bg-background">
                          ...
                          <div className="flex gap-2 mt-3 mb-3">
                            ...
                            <StatusIndicator ok={t.medidas_corregidas} label="Medidas" />
                            ...
                            {(t.piezas_data || (Array.isArray(t.piezas) ? t.piezas : [])).map((p, idx) => (
                              <div key={idx} className="border rounded-lg p-3 bg-muted/20">
                                <div className="text-sm font-medium">
                                  {formatDualUnit(normalizeToMm(p.largo ?? p.h))} × {formatDualUnit(normalizeToMm(p.ancho ?? p.w))}
                                </div>
