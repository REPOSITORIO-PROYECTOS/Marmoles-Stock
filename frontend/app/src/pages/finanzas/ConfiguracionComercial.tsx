...
export function ConfiguracionComercial() {
  ...
  const cargarCatalogos = async () => {
    setLoading(true);
    try {
      const [resDescuentos, resExtras] = await Promise.all([
        get<Servicio[]>('/api/servicios?categoria=DescuentoPresupuesto'),
        get<Servicio[]>('/api/servicios?categoria=ExtraPresupuesto'),
      ]);
      setDescuentos(Array.isArray(resDescuentos) ? resDescuentos : []);
      setExtras(Array.isArray(resExtras) ? resExtras : []);
      setEditMap({});
      setPageDescuentos(1);
      setPageExtras(1);
    } catch (err) {
  ...
  const eliminarItem = async (id: string) => {
    ...
    try {
      ...
      toast.error('No se pudo eliminar (requiere perfil admin)');
    }
  };
  ...
  const extrasFiltrados = useMemo(() => {
    ...
    return extras.filter((e) => {
      const txt = `${e.nombre} ${e.unidad || ''} ${e.precio_base}`.toLowerCase();
      return txt.includes(term);
    });
  }, [extras, searchExtras]);
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        <div>
          ...
          <p className="text-sm text-muted-foreground">Gestione catálogos de descuentos y extras para el cotizador.</p>
        </div>
