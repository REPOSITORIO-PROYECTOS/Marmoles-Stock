      ...
      <div ref={containerRef} className="w-full flex justify-center bg-gray-100 p-4 rounded-xl">
        ...
        >
          {placements.map((p, idx) => (
            <div
              key={idx}
              className="absolute border border-cyan-700 bg-cyan-500/20 hover:bg-cyan-500/40 transition-colors flex items-center justify-center overflow-hidden"
              style={{
            ...
            >
              <div className="text-center p-1">
                <div className="font-black text-cyan-900 leading-none" style={{ fontSize: Math.max(8, Math.min(p.w * scale / 3, 14)) + 'px' }}>
                  #{idx + 1}
                </div>
                {p.w * scale > 40 && (
                  <div className="text-[8px] font-bold text-cyan-800 opacity-80 uppercase truncate px-1">
                ...
                {items.map((p, idx) => {
                  ...
                  return (
                    <div key={idx} className="flex justify-between text-sm text-gray-600">
                      <span><strong className="text-cyan-600">#{originalIndex}</strong> {p.w}x{p.h} cm</span>
