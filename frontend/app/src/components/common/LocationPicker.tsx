...
function LocationMarker({ position, setPosition, onManualPick }: { position: L.LatLng | null, setPosition: (p: L.LatLng) => void, onManualPick?: () => void }) {
    useMapEvents({
        click(e) {
            onManualPick?.();
...
export function LocationPicker({ onConfirm, onCancel, initialCoordinates }: LocationPickerProps) {
    ...
    const googlePlaceIdRef = useRef<string>('');
        ...
        if (googleReady && inputRef.current && (window as any).google?.maps?.places) {
            ...
            autocomplete.addListener('place_changed', () => {
                ...
                if (geometry) {
                    ...
                    googlePlaceIdRef.current = String(place.place_id || '');
                    setPosition(new L.LatLng(lat, lng));
    ...
    const handleSearch = async (e?: React.FormEvent) => {
        ...
        try {
            ...
            if (data && data.length > 0) {
                ...
                googlePlaceIdRef.current = `osm:${lat.toFixed(6)}:${lon.toFixed(6)}`;
    ...
    const handleConfirm = async () => {
        ...
        try {
            ...
            const pid = (googlePlaceIdRef.current || '').trim() || `manual:${position.lat.toFixed(6)}:${position.lng.toFixed(6)}`;
            onConfirm({
            ...
            <div className="relative flex-1 rounded-md overflow-hidden border h-full">
                ...
                {googleReady && (window as any).google?.maps ? (
                    ...
                    >
                        ...
                        />
                        <LocationMarker position={position} setPosition={setPosition} onManualPick={() => { googlePlaceIdRef.current = ''; }} />
